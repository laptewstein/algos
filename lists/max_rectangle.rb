class Solution
  ...
  def self.largest_rectangle_areas(array, *index_sequence)
    ...
    array.each_with_index do |current_height, index|
      ...
      #   - calculate it's area by
      #      (height) multiplying element's value
      #      (width) by the offset from current index
      while current_height < array[index_sequence.last]
        height = array[index_sequence.pop]
        width = index.pred - index_sequence.last #  0 - (-1) => 1
        areas << height * width
        puts("[!OMG!] index_sequence.pop of #{height}, width (#{index.pred} (index) - #{index_sequence.last} (index_sequence.last)) index_sequence #{index_sequence}, #{height} * #{width} => #{areas}")
      end  
      index_sequence << index
      puts("[#{index}] hve been referenced in the sequence: #{index_sequence}")
    end
    areas
  end
end
